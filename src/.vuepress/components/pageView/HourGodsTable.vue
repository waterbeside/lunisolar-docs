...
function init() {
  const godData = gods.hour[props.godName !=='旬空' ? '时神随月将及日干支者' : '时神随日六旬者'][props.godName]
  if (!godData) return
...
<template>
  <table class="godsTable" :style="{'--v-item-width': props.vItemWidth + 'px', '--v-item-padding-x': props.vItemPaddingX + 'px', '--key-width': keyWidth + 'px'}">
    <thead>
      <tr>
        <th>{{ first }}</th>
    ...
    <tbody>
      <tr v-for="(item, index) in list" :key="item.key">
        ...
        <td v-for="v, i in item.content" class="content-item" :class="{dx: props.godName === '貴登天門' && i === 0}">
